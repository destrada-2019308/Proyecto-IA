...
try:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT receta, ingredientes, instrucciones FROM recetas")
    rows = cursor.fetchall()
    df = pd.DataFrame(rows, columns=["receta", "ingredientes", "instrucciones"])
...
def recomendar_receta():
    ...
    try:
    # Reproducir sonido al presionar el botón sin interrumpir música de fondo
        try:
            efecto = pygame.mixer.Sound("audio_receta.ogg")  # Cargar sonido como efecto separado
            efecto.play()  # Reproducir efecto sin afectar la música
        except Exception as e:
